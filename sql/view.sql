...
create view show_who_took_the_book as
select s.name  from orders
    join books b on orders.book_id = b.id
    join authors a on a.id = b.author_id
    join students s on orders.student_id = s.id
where b.name = 'Капитанская дочка';

select * from show_who_took_the_book
